...
```
- Input: email, password, full_name, phone, address (optional), resume (file), portfolio (files)
...
- Process:
  ...
  2. Create record in users table (email, temporary_password='PENDING', role='job_seeker', is_active=FALSE)
  3. Create record in job_seekers_profiles (user_id, full_name, phone, address, verified_at)
  4. Update pending_applications.application_status = 'approved'
  5. Set reviewed_at = NOW(), reviewed_by = admin_id
  6. Send email to applicant with account activation link
...
- Process:
  ...
  2. Add rejection_reason to record
